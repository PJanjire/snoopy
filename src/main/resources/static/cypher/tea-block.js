...
Tea.encrypt = function(plaintext, password) {
    ...
    var encCipherText = Base64.encode(ciphertext);
    encCipherText = encCipherText.replace(/\+/g , "-");
    encCipherText = encCipherText.replace(/\//g , "_");
    encCipherText = encCipherText.replace(/=/g , ".");
...
Tea.longsToStr = function(l) {  // convert array of longs back to string
    ...
    for (var i=0; i<l.length; i++) {
        ...
        //console.log(i+': '+l[i]+' = '+l[i].toString(16)+' - '+(l[i] & 0xFF)+' '+(l[i]>>>8 & 0xFF)+' '+(l[i]>>>16 & 0xFF)+' '+(l[i]>>>24 & 0xFF)+' ');
    }
