...
public class GlobalExceptionHandler {
    ...
    // ✅ Handle .map file requests silently
    ...
    public Object handle401(SecurityException ex, HttpServletRequest request) {
        if (isAjax(request)) {
            return new ResponseEntity<>(
                    new ApiError(401, ex.getMessage(), request.getRequestURI()),
    ...
    public Object handle404(ResourceNotFoundException ex, HttpServletRequest request) {
        if (isAjax(request)) {
            return new ResponseEntity<>(
                    new ApiError(404, ex.getMessage(), request.getRequestURI()),
    ...
    public Object handle500(Exception ex, HttpServletRequest request) {
        ...
        if (isAjax(request)) {
            return new ResponseEntity<>(
                    new ApiError(500, "Internal Server Error", request.getRequestURI()),
